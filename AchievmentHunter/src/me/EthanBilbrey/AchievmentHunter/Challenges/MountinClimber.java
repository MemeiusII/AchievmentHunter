//I messed up the name, so disregard the name
...
public class MountinClimber implements Challenge
{
	...
	public void sendStartMessage() {
		...
		{
			p.sendMessage(ChatColor.YELLOW + "Cave Crawler");
			p.sendMessage(ChatColor.YELLOW + "-" + ChatColor.WHITE + " Go to Y = 25");
		}
	}
	...
	public void startTimer() {
		...
		taskId = Bukkit.getScheduler().scheduleSyncRepeatingTask(Main.getPlugin(Main.class), () -> {
			...
			else if(ChallengeManager.player.getLocation().getBlockY() <= 25) 
			{
